...
namespace SickDev.DevConsole
{
	public class DevConsole : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		public float height
		{
			get { return Mathf.Clamp(settings.preferredHeight, 0, 1 - openButton.height * settings.scale / Screen.height) * Screen.height / settings.scale; }
		}
		...
		private static DevConsole _singleton;
		...
		private void OnCommandSystemExceptionThrown(Exception exception)
		{
		...
		private void OnCommandSystemMessage(string message)
		{
			EntryData entry = new EntryData();
			entry.text = message;
		...
		private void ExecuteIfCommand(string text)
		{
		...
		private string ConvertCommandResultToString(object result)
		{
			if (result == null)
				return "null";
